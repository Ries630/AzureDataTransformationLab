...
現Azureの上限は1である。連続curlでは、約212 msのHTTP 200、約60,092 msのHTTP 503、約200 msのHTTP 200が観測された。Node実装でも同様の並びがあり、単発要求と通常ハンドラーの別経路では`VALID`を確認できた。認証設定だけを変更した後も同じ失敗が残っているため、上限1が主因である可能性が最も高い。ただし、上限40での比較はまだ実施しておらず、原因は確定していない。
