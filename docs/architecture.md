...
FunctionはLinuxのFlex Consumption、Azure Functionsランタイムv4、Python 3.14を対象とする。インスタンスメモリは512 MB、最大インスタンス数は1、HTTPトリガーのインスタンスあたり同時実行数は1、Always Readyは0とする。Function専用のホスト・デプロイStorageを入力Storageから分離し、Identityベースの接続を使う。構成の判断理由と未検証事項は[ADR-0004](adr/0004-separate-function-host-storage.md)を参照する。
